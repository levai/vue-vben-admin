...
CREATE TABLE IF NOT EXISTS `sys_operation_log` (
    ...
    `operation_type` VARCHAR(20) NOT NULL COMMENT '操作类型：view/add/edit/delete/export/import等',
    `operation_module` VARCHAR(50) COMMENT '操作模块：system/user/menu等',
    `operation_page` VARCHAR(200) COMMENT '操作页面路径',
    `operation_title` VARCHAR(200) COMMENT '操作标题/描述',
    `request_method` VARCHAR(10) COMMENT '请求方法：GET/POST/PUT/DELETE',
    `request_url` VARCHAR(500) COMMENT '请求URL',
    `request_params` TEXT COMMENT '请求参数（JSON格式）',
    `response_code` INT COMMENT '响应状态码',
    `response_data` TEXT COMMENT '响应数据（JSON格式，可选）',
...
### 3.3 Controller (SystemOperationLogController.java)

**文件位置**：`apps/backend-service/src/main/java/com/vben/admin/controller/SystemOperationLogController.java`

**API接口**：

- `POST /system/operation-log` - 创建操作日志（批量）
- `GET /system/operation-log/list` - 获取日志列表（分页、筛选）
- `GET /system/operation-log/{id}` - 获取日志详情
- `DELETE /system/operation-log/{id}` - 删除日志
- `DELETE /system/operation-log/batch` - 批量删除日志
- `GET /system/operation-log/statistics` - 获取日志统计信息
...
- `getOperationLogStatistics(params)` - 获取统计信息（可选）
...
- 日志列表展示（使用 VbenVxeGrid）
- 筛选条件：
  - 用户（下拉选择，支持用户名/真实姓名搜索）
  - 操作类型（多选：view/add/edit/delete等）
...
```typescript
{
  ...
  meta: {
    ...
    authority: ['admin', 'system:operation-log:view'],
  },
...
OPERATION_LOG: {
  ...
  EXPORT: 'ac:system:operation-log:export',
}
```
...
- 为常用查询字段建立索引（userId、operationType、operationModule、createTime、status）
- 复合索引优化查询性能
...
4. `apps/backend-service/src/main/java/com/vben/admin/controller/SystemOperationLogController.java` - Controller
...
12. `apps/backend-service/src/main/java/com/vben/admin/core/task/OperationLogCleanupTask.java` - 定时清理任务
13. `apps/backend-service/src/main/resources/db/schema.sql` - 添加表结构
14. `apps/backend-service/src/main/java/com/vben/admin/BackendServiceApplication.java` - 启动类（添加@EnableScheduling）
...
4. `apps/web-antd/src/router/routes/modules/system.ts` - 路由配置（修改）
5. `apps/web-antd/src/constants/permission-codes.ts` - 权限码（修改）
6. `apps/web-antd/src/locales/langs/zh-CN/system.json` - 中文翻译（修改）
7. `apps/web-antd/src/locales/langs/en-US/system.json` - 英文翻译（修改）

## 七、实施步骤

1. **数据库设计**：创建操作日志表（sys_operation_log）
2. **后端实体和DTO**：创建实体类、DTO、VO
3. **后端工具类**：实现操作信息解析、浏览器信息解析、敏感信息过滤
4. **后端AOP切面**：实现自动记录所有API请求的切面（核心）
5. **后端Service和Mapper**：实现日志的存储、查询、删除业务逻辑
6. **后端定时任务**：实现清理3个月前的日志任务
