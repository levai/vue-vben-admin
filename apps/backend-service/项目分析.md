...
- ✅ **用户列表** (`GET /system/user`)
  - 分页查询
  - 多条件搜索（用户名、真实姓名、部门、状态）
  - 时间范围查询（创建时间）
  - 排序（按创建时间倒序）

- ✅ **用户详情** (`GET /system/user/{id}`)
  - 根据ID获取用户详细信息
...
- ✅ **启用/禁用用户** (`PUT /system/user/{id}/status`)
  - 更新用户状态

- ✅ **重置密码** (`PUT /system/user/{id}/password`)
  - 重置用户密码（BCrypt 加密）

- ✅ **用户选项列表** (`GET /system/user/options`)
  - 用于下拉选项
  - 支持条件查询和 limit 限制

### 3. 角色管理模块

- ✅ **角色列表** (`GET /system/role`)
  - 分页查询
  - 多条件搜索（名称、ID、备注、状态）
  - 时间范围查询（创建时间）
  - 排序（按创建时间倒序）

- ✅ **角色详情** (`GET /system/role/{id}`)
  - 根据ID获取角色详细信息
...
- ✅ **角色选项列表** (`GET /system/role/options`)
  - 用于下拉选项
  - 支持条件查询和 limit 限制

### 4. 菜单管理模块

- ✅ **菜单列表** (`GET /system/menu`)
  - 树形结构返回
  - 状态过滤
  - 排序（按 sort_order）

- ✅ **菜单详情** (`GET /system/menu/{id}`)
  - 根据ID获取菜单详细信息
...
- ✅ **批量更新菜单排序** (`PUT /system/menu/batch-order`)
  - 用于拖拽排序
  - 批量更新菜单的排序和父级关系
...
- ✅ **部门列表** (`GET /system/dept`)
  - 树形结构返回
  - 包含创建时间

- ✅ **部门详情** (`GET /system/dept/{id}`)
  - 根据ID获取部门详细信息
...
- ✅ **操作日志列表** (`GET /system/operation-log`)
  - 分页查询
  - 多条件筛选（用户、操作类型、操作模块、状态、时间范围）
  - 搜索关键词（模糊查询）
  - 排序（按创建时间倒序）

- ✅ **操作日志详情** (`GET /system/operation-log/{id}`)
  - 根据ID获取操作日志详细信息
  - 包含请求参数、响应数据、浏览器信息等

- ✅ **删除操作日志** (`DELETE /system/operation-log/{id}`)
  - 逻辑删除单条日志

- ✅ **批量删除操作日志** (`DELETE /system/operation-log/batch`)
  - 批量逻辑删除日志

- ✅ **操作类型列表** (`GET /system/operation-log/types`)
  - 用于下拉选项
  - 返回全部数据，支持搜索关键词过滤

- ✅ **操作模块列表** (`GET /system/operation-log/modules`)
  - 用于下拉选项
  - 返回全部数据，支持搜索关键词过滤

- ✅ **自动日志记录**:
  - AOP 切面自动记录所有 `@RestController` 方法调用
  - 记录用户信息、操作类型、操作模块、请求参数、响应数据等
  - 异步保存，不影响业务性能
  - 敏感信息过滤（密码、token等）
...
- ✅ **QueryHelper**: 查询辅助工具类
  - `applyTimeRange()` - 时间范围查询
  - `applyLimit()` - Limit 限制
  - `getValidLimit()` - 获取有效 limit 值
- ✅ **OperationInfoParser**: 操作信息解析
  - 从请求 URL 和方法推断操作类型和模块
- ✅ **BrowserInfoParser**: 浏览器信息解析
...
这是一个**标准的企业级 Spring Boot 后端服务**，采用**分层架构**和**RESTful API 设计**，实现了完整的**用户、角色、菜单、部门、操作日志管理**功能，支持**RBAC 权限模型**和**JWT 认证**。代码风格**规范统一**，使用了**Lombok**简化代码，**MyBatis Plus**简化数据库操作，**全局异常处理**统一错误响应，**AOP 切面**自动记录操作日志，是一个**生产可用**的后端服务项目。

### RESTful API 设计规范

所有系统模块统一使用 RESTful 资源根路径：

- ✅ **用户管理**: `GET /system/user` - 列表
- ✅ **角色管理**: `GET /system/role` - 列表
- ✅ **菜单管理**: `GET /system/menu` - 列表
- ✅ **部门管理**: `GET /system/dept` - 列表
- ✅ **操作日志**: `GET /system/operation-log` - 列表

所有接口遵循 RESTful 规范，通过 HTTP 方法区分操作（GET/POST/PUT/DELETE），不使用 `/list` 后缀。
